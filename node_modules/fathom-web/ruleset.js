...
class Ruleset {
    ...
    constructor(rules, coeffs = [], biases = []) {
        ...
        for (let rule of rules) {
            if (rule instanceof _rule.InwardRule) {
                ...
                for (let type of emittedTypes) {
                    (0, _utilsForFrontend.setDefault)(this._rulesThatCouldEmit, type, () => []).push(rule);
                }
                for (let type of rule.typesItCouldAdd()) {
                    (0, _utilsForFrontend.setDefault)(this._rulesThatCouldAdd, type, () => []).push(rule);
                }
            } else if (rule instanceof _rule.OutwardRule) {
     ...
     */
    against(doc) {
        return new BoundRuleset(doc, this._inRules, this._outRules, this._rulesThatCouldEmit, this._rulesThatCouldAdd, this._coeffs, this.biases);
    }
...
class BoundRuleset {
    ...
    get(thing) {
        if (typeof thing === 'string') {
        ...
        } else if ((0, _utilsForFrontend.isDomElement)(thing)) {
            ...
            // cache hit.
            const outRule = (0, _rule.rule)(thing, (0, _rhs.out)(Symbol('outKey')));
    ...
    weightedScore(mapOfScores) {
        ...
        for (const [name, score] of mapOfScores) {
            total += score * (0, _utilsForFrontend.getDefault)(this._coeffs, name, () => 1);
        }
        return total;
    }
    ...
    _prerequisitesTo(rule, undonePrereqs = new Map()) {
        for (let prereq of rule.prerequisites(this)) {
            if (!this.doneRules.has(prereq)) {
                ...
                (0, _utilsForFrontend.setDefault)(undonePrereqs, prereq, () => []).push(rule);
    ...
    _execute(rule) {
        ...
        let sorted;
        try {
            sorted = [rule].concat((0, _utilsForFrontend.toposort)(prereqs.keys(), prereq => prereqs.get(prereq)));
        } catch (exc) {
            if (exc instanceof _exceptions.CycleError) {
                throw new _exceptions.CycleError('There is a cyclic dependency in the ruleset.');
            } else {
                throw exc;
            }
        }
        let fnodes;
        for (let eachRule of (0, _utilsForFrontend.reversed)(sorted)) {
    ...
    inwardRulesThatCouldEmit(type) {
        return (0, _utilsForFrontend.getDefault)(this._rulesThatCouldEmit, type, () => []);
    }

    /** @return {Rule[]} */
    inwardRulesThatCouldAdd(type) {
        return (0, _utilsForFrontend.getDefault)(this._rulesThatCouldAdd, type, () => []);
    }
    ...
    fnodeForElement(element) {
        return (0, _utilsForFrontend.setDefault)(this.elementCache, element, () => new _fnode.Fnode(element, this));
    }
}