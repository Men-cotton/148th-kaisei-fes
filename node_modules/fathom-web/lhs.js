...
class DomLhs extends Lhs {
    ...
    typesMentioned() {
        return new _utilsForFrontend.NiceSet();
    }
}
...
class TypeLhs extends Lhs {
    ...
    fnodes(ruleset) {
        const cached = (0, _utilsForFrontend.getDefault)(ruleset.typeCache, this._type, () => []);
    ...
    typesMentioned() {
        return new _utilsForFrontend.NiceSet([this._type]);
    }
}
...
class TypeMaxLhs extends AggregateTypeLhs {
    ...
    fnodes(ruleset) {
        ...
        return (0, _utilsForFrontend.setDefault)(ruleset.maxCache, this._type, function maxFnodesOfType() {
            return (0, _utilsForFrontend.maxes)(getSuperFnodes(), fnode => ruleset.weightedScore(fnode.scoresSoFarFor(self._type)));
        });
    }
}
...
class BestClusterLhs extends AggregateTypeLhs {
    ...
    fnodes(ruleset) {
        ...
        // Cluster them:
        const clusts = (0, _clusters.clusters)(fnodesOfType, this._options.splittingDistance, (a, b) => (0, _clusters.distance)(a, b, this._options));
        // Tag each cluster with the total of its nodes' scores:
        const clustsAndSums = clusts.map(clust => [clust, (0, _utilsForFrontend.sum)(clust.map(fnode => fnode.scoreFor(this._type)))]);
        // Return the highest-scoring cluster:
        return (0, _utilsForFrontend.max)(clustsAndSums, clustAndSum => clustAndSum[1])[0];
    }
}
...
class AndLhs extends Lhs {
    ...
    *fnodes(ruleset) {
    ...
    typesMentioned() {
        return new _utilsForFrontend.NiceSet(this._args.map(arg => arg.guaranteedType()));
    }
}
...
class NearestLhs extends Lhs {
    ...
    *fnodes(ruleset) {
        ...
        if (bs.length > 0) {
            ...
            for (const a of as_) {
                const nearest = (0, _utilsForFrontend.min)(bs, b => this._distance(a, b));
                yield { fnode: a,
    ...
    possibleTypeCombinations() {
        return [new _utilsForFrontend.NiceSet([this._a.guaranteedType()])];
    }

    typesMentioned() {
        return new _utilsForFrontend.NiceSet([this._a.guaranteedType(), this._b.guaranteedType()]);
    }
