...
exports.inlineTexts = inlineTexts;
exports.inlineTextLength = inlineTextLength;
exports.collapseWhitespace = collapseWhitespace;
exports.linkDensity = linkDensity;
...
function best(iterable, by, isBetter) {
    ...
    let isFirst = true;
    _wu2.default.forEach(function (item) {
        const key = by(item);
...
function sum(iterable) {
    let total;
    let isFirst = true;
    _wu2.default.forEach(function assignOrAdd(addend) {
        if (isFirst) {
            total = addend;
            isFirst = false;
        } else {
...
const blockTags = new Set(['ADDRESS', 'BLOCKQUOTE', 'BODY', 'CENTER', 'DIR', 'DIV', 'DL', 'FIELDSET', 'FORM', 'H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'HR', 'ISINDEX', 'MENU', 'NOFRAMES', 'NOSCRIPT', 'OL', 'P', 'PRE', 'TABLE', 'UL', 'DD', 'DT', 'FRAMESET', 'LI', 'TBODY', 'TD', 'TFOOT', 'TH', 'THEAD', 'TR', 'HTML']);
/**
...
function* inlineTexts(element, shouldTraverse = element => true) {
    ...
    for (let child of walk(element, element => !(isBlock(element) || element.tagName === 'SCRIPT' && element.tagName === 'STYLE') && shouldTraverse(element))) {
...
function inlineTextLength(element, shouldTraverse = element => true) {
    return sum(_wu2.default.map(text => collapseWhitespace(text).length, inlineTexts(element, shouldTraverse)));
}
...
function toposort(nodes, nodesThatNeed) {
    ...
    function visit(node) {
        if (inProgress.has(node)) {
            throw new _exceptions.CycleError('The graph has a cycle.');
        }
        if (todo.has(node)) {
            inProgress.add(node);
...
function isVisible(fnodeOrElement) {
    ...
    for (const ancestor of ancestors(element)) {
        ...
        if (style.visibility === 'hidden' || style.display === 'none' || style.opacity === '0' || style.width === '0' || style.height === '0') {
            return false;
        } else {
            ...
            if (rect.right + frame.scrollX < 0 || rect.bottom + frame.scrollY < 0) {
                return false;
            }
        }
    }
