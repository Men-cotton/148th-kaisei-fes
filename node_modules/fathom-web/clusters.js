...
function numStrides(left, right) {
    ...
    while (shouldContinue) {
        ...
        if ((shouldContinue = sibling && sibling !== right) && !(0, _utilsForFrontend.isWhitespace)(sibling)) {
            num += 1;
        }
    }
    if (sibling !== right) {
        ...
        while (sibling) {
            ...
            if (sibling && !(0, _utilsForFrontend.isWhitespace)(sibling)) {
                num += 1;
            }
        }
    }
 ...
 *
 */
function distance(fnodeA, fnodeB, { differentDepthCost = 2,
    differentTagCost = 2,
    sameTagCost = 1,
    strideCost = 1,
    ...
    const elementA = (0, _utilsForFrontend.isDomElement)(fnodeA) ? fnodeA : fnodeA.element;
    const elementB = (0, _utilsForFrontend.isDomElement)(fnodeB) ? fnodeB : fnodeB.element;
...
function euclidean(fnodeA, fnodeB) {
    ...
    const elementA = (0, _utilsForFrontend.toDomElement)(fnodeA);
    const elementB = (0, _utilsForFrontend.toDomElement)(fnodeB);
...
class DistanceMatrix {
    ...
    closest() {
        ...
        return (0, _utilsForFrontend.min)(clustersAndDistances(), x => x.distance);
    }
    ...
    clusters() {
        ...
        return Array.from(this._matrix.keys()).map(e => Array.from(_wu2.default.flatten(false, e)));
    }
}
